import { getDatabase } from '../database.js';
    ...
    export function getModerationCasesForUser(
      ...
      const conditions = ['guild_id = ?','user_id = ?'];
      const parameters = [guildId,userId];

      if (!includeRemoved) conditions.push("status != 'removed'");
      if (caseType) {
        validateRequiredString(caseType,'caseType');
    ...
    export function removeModerationCase({
      ...
      if(existingCase.status==='removed'){
        throw new Error(`Moderation case #${caseNumber} has already been removed.`);
      }
    ...
    function getNextCaseNumberWithinTransaction(database,guildId){
      const row=database.prepare(`
          SELECT COALESCE(MAX(case_number), 0) + 1 AS next_case_number
          FROM moderation_cases
          WHERE guild_id = ?
        `).get(guildId);
    ...
    function validateCaseNumber(caseNumber){
      if(!Number.isInteger(caseNumber)||caseNumber<1){
        throw new TypeError('caseNumber must be a positive integer.');
      }
    }
